...
_tax = _value * LIFE_SETTINGS(getNumber,"civRDMed_transferTax");
...
if (LIFE_SETTINGS(getNumber,"player_moneyLog") isEqualTo 1) then {
    if (LIFE_SETTINGS(getNumber,"battlEye_friendlyLogging") isEqualTo 1) then {
        money_log = format [localize "STR_DL_ML_transferredcivRDMed_BEF",_value,_unit getVariable ["realname",name _unit],[civRDMed] call life_fnc_numberText,[civRDM] call life_fnc_numberText];
    } else {
        money_log = format [localize "STR_DL_ML_transferredcivRDMed",profileName,(getPlayerUID player),_value,_unit getVariable ["realname",name _unit],[civRDMed] call life_fnc_numberText,[civRDM] call life_fnc_numberText];
    };
