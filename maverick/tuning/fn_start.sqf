...
if (vehicle player == player) exitWith {["You have to be in a vehicle",false,"slow"] call life_fnc_notificationSystem;};
if (driver vehicle player != player) exitWith {["You're not the driver of the vehicle!",false,"slow"] call life_fnc_notificationSystem;};
if ((count crew vehicle player) > 1) exitWith {["You have to be alone in the vehicle!",false,"slow"] call life_fnc_notificationSystem;};
if (isNil {(vehicle player) getVariable "vehicleID"}) exitWith {["The vehicle has to be stored in a garage first before it can be tuned!",false,"slow"] call life_fnc_notificationSystem;};
if (diag_tickTime - mav_tuning_lastVisit < 10) exitWith {["You are already visiting the tuning shop!",false,"slow"] call life_fnc_notificationSystem;};
...
// Not tuneable?
if (_configName == "") exitWith {["This vehicle cannot be tuned",false,"slow"] call life_fnc_notificationSystem;};
...
if ((count crew vehicle player) > 1) exitWith {
	["You have to be alone in the vehicle!",false,"slow"] call life_fnc_notificationSystem;
...
// Hide hints and chat
showChat false;
["",false,"slow"] call life_fnc_notificationSystem;
