...
#define civRDM (missionNamespace getVariable getText (MAVCFG >> "cashVar"))
...
switch _mode do {
    case "onLoad": {
		...
		_ctrlCashOnHand ctrlSetText format [getText (MAVCFG >> "localization" >> "msgCashOnHand"), SYMBOL, [civRDM] call BIS_fnc_numberText];
    ...
    case "onTreeChanged": {
		...
		private _afford = (MAVV(Total) + _price) > civRDM;
	...
	case "complete": {
		...
		missionNamespace setVariable [getText (MAVCFG >> "cashVar"), (civRDM - _cash)];
