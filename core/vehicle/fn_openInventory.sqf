...
if (life_is_processing) exitWith {["You cannot access a vehicle while processing.",true,"slow"] call life_fnc_notificationSystem;};
if ((_vehicle getVariable ["trunk_in_use",false])) exitWith {hint localize "STR_MISC_VehInvUse"};
//if (isNull (_vehicle getVariable "vehicle_info_owners" select 0)) exitWith {};
//if ((_vehicle getVariable "vehicle_info_owners" select 0) != player) exitWith {};
["Opening inventory, Please wait...",true,"slow"] call life_fnc_notificationSystem;
uiSleep random 10;
if (life_is_processing) exitWith {["You cannot access a vehicle while processing.",true,"slow"] call life_fnc_notificationSystem;};
if ((_vehicle getVariable ["trunk_in_use",false])) exitWith {hint localize "STR_MISC_VehInvUse"};

_unitsnear = (nearestObjects[_vehicle, ["Man"], 20]) arrayIntersect playableUnits;
if(count _unitsnear > 1) exitWith {["Sorry, you cannot open your inventory to your vehicle when players are nearby!",true,"slow"] call life_fnc_notificationSystem;};
