...
*/
private["_failedCrafting","_price","_dialog","_item","_site","_vehicle","_itemInfo","_oldItem","_newItem","_upp","_itemName","_ui","_progress","_pgText","_cP","_allMaterial","_matsNeed","_invSize","_handledItem","_itemFilter","_backpackOldItems","_weight","_nearCar","_vehicleCheck","_trunkCar","_trunk","_vehCheck","_invAmount","_weight"];
...
if ((missionNamespace getVariable ['mav_ttm_var_crafting',0]) isEqualTo 0 ) exitwith {["You need to unlock the crafting perk before you can use this toolbench!",true,"slow"] call life_fnc_notificationSystem;}; 
...
//Some checks
if(_price > civRDM) exitWith {["You cannot afford to craft this.",true,"slow"] call life_fnc_notificationSystem;};
...
while{true} do
{
	...
	if (((getMarkerPos "CraftingArea1") distance player) > 15 && ((getMarkerPos "CraftingArea1_1") distance player) > 15 ) exitWith {
    for [{_i=0},{_i<_invSize-1},{_i=_i+2}] do {
        _handledItem = M_CONFIG(getText,"VirtualItems",_oldItem select _i,"variable");
        [true,_handledItem,_oldItem select _i+1] call life_fnc_handleInv;
	    };
	civRDM = civRDM + _price;
	_failedCrafting = true;
	};
};
if (_failedCrafting) exitWith {["Failed to craft Item!",false,"slow"] call life_fnc_notificationSystem;  life_is_processing = false; }; 
...
if(_itemFilter isEqualTo "weapon") then{
	...
	if(player canAdd _newItem) then{
		...
		if(currentWeapon player isEqualTo "") then{
			...
			5 cutText ["","PLAIN"];
			["VolticRP","Crafting was cancelled because you do not have the storage",nil,"notification",true,"slow"] call life_fnc_notificationSystem;
			for [{_i=0},{_i<_invSize-1},{_i=_i+2}] do {
				_handledItem = M_CONFIG(getText,"VirtualItems",_oldItem select _i,"variable");
				[true,_handledItem,_oldItem select _i+1] call life_fnc_handleInv;
			};
