...
life_deathCamera camSetFOV 1.5;
...
if(isNull _killer || _killer == player) then {
	_Killedby ctrlSetText format ["Killed by: %1 (%2) (%3)",_unit getVariable["realname",""],_groupName,getPlayerUID _killer];
} else {
	_Killedby ctrlSetText format ["Killed by: %1 (%2) (%3)",_killer getVariable["realname",""],_groupNameKiller,getPlayerUID _killer];
};
[format["%1 was killed by %2 (%3) (%4)", profileName, _killer getVariable["realname",""],_groupName,getPlayerUID _killer], "KillLogging"] remoteExecCall ["A3Log", 2];
