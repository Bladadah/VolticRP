...
if ((isClass (missionConfigFile >> "Housing" >> "Malden" >> (typeOf _curObject)) && {player distance _curObject < 12}) || (isClass (missionConfigFile >> "Garages" >> "Malden" >> (typeOf _curObject)) && {player distance _curObject < 12}) || ((nearestObject [_pos,"Land_Dome_Small_F"]) isEqualTo _curObject || (nearestObject [_pos,_vaultHouse]) isEqualTo _curObject)) exitWith {
