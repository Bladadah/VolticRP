...
switch (_code) do {
    ...
    case 74: {
    if (_shift) then {
        hint "Your action has been logged, administrators will be investigating your account for suspected malicious activities, if you have done this by accident, report it to an admin with the code [653] have a nice day!";
	...
	case 34: {
        ...
        {
            if((animationState cursorObject) != "Incapacitated" && !life_knockout && !(player getVariable["restrained",false]) && !(player getVariable["tied",false]) && !life_istazed && (stance player) != "PRONE") then
            {
