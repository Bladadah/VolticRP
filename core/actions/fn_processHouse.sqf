...
if (life_inv_MDMA > 0) then {
	_x = life_inv_MDMA;
	if(!([false,"MDMA",_x] call life_fnc_handleInv)) exitWith {5 cutText ["","PLAIN"];};
	if(!([true,"ecstasy",_x] call life_fnc_handleInv)) exitWith {5 cutText ["","PLAIN"]; [false,"MDMA",_x] call life_fnc_handleInv;};
};
