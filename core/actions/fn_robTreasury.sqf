...
if ((missionNamespace getVariable ['mav_ttm_var_satellite',0]) isEqualTo 0) exitwith {["You need to complete the Secrets of the depths quest!",true,"slow"] call life_fnc_notificationSystem;}; 
if(typeOf _vault == "Land_CargoBox_V1_F" && time - (fed_bank getVariable ["reset",time]) < 3600) exitWith {hint "A major crime has recently taken place, please wait 1 hour from the last major crime!";};
...
_kassa = 280000 + round(random 130000);
_shop removeAction _action;
[1,format["ALARM! - %1 Robbery in progress", _shop]] remoteExec ["life_fnc_broadcast",west]; 
[3] remoteExec ['life_fnc_AAN_Bank',-2];
disableSerialization;
5 cutRsc ["life_progress","PLAIN"];
_ui = uiNameSpace getVariable "life_progress";
_progress = _ui displayCtrl 38201;
_pgText = _ui displayCtrl 38202;
_pgText ctrlSetText format["Robbing, empty your inventory for the loot, stay within 25 meters (1%1)...","%"];
_progress progressSetPosition 0.01;
_cP = 0.01;

[1,"The Underwater Treasury is being robbed!"] remoteExec ["life_fnc_broadcast",0]; // General broadcast alert to everyone, uncomment for testing, or if you want it anyway.

while{true} do
{
	uiSleep 11;
	_cP = _cP + (0.006 * (missionNamespace getVariable ["mav_ttm_var_robbingMultiplier", 1]));
	_progress progressSetPosition _cP;
	_pgText ctrlSetText format["Robbing, empty your inventory for the loot, stay within 25 meters (%1%2)...",round(_cP * 100),"%"];
...
5 cutText ["","PLAIN"];
titleText[format["You have recieved $%1 and some loot from the robbery, Run!!",[_kassa] call life_fnc_numberText],"PLAIN"];
[true,_reward,8] call life_fnc_handleInv;
[true,"relicFed",1] call life_fnc_handleInv;
[true,"plans",3] call life_fnc_handleInv;
...
if!(alive _robber) exitWith {};
[0,format["Police News: The bank has just been robbed: the stolen ammount of money was $%3 and some other loot!",name _robber, _shop, [_kassa] call life_fnc_numberText]] remoteExec ["life_fnc_broadcast",west];
[0,format["Police News: The bank has just been robbed: the stolen ammount of money was $%3 and some other loot, if you have seen the robbers, contact police!",name _robber, _shop, [_kassa] call life_fnc_numberText]] remoteExec ["life_fnc_broadcast",civilian];
